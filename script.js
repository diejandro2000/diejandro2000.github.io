...
function generatePDF() {
  ...
  let logoBase64 = clientId === "DOS40" ? dos40Logo : null;
  ...
  if (logoBase64) {
    try {
      ...
      const imgY = y - 30; // aligned properly
      doc.addImage(logoBase64, "PNG", imgX, imgY, imgWidth, imgHeight);
    } catch (err) {
      console.warn("⚠️ Could not embed client logo:", err);
    }
  }
  ...
  document.querySelectorAll("#invoiceItems tr").forEach(row => {
    ...
    let combinedText = (manualTotal + totalExtras).toFixed(2) + " €";
    if (extraLines.length > 0) {
      combinedText += "\n" + extraLines.join("\n");
    }
